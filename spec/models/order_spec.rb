...
RSpec.describe Order do
  ...
  describe 'instance methods' do
    before :each do
      ...
      @order_1 = @user.orders.create!(status: "packaged", address: @address)
  ...
  describe 'class methods' do
    before :each do
      @user = User.create!(name: 'Megan', email: 'megan_1@example.com', password: 'securepassword')
      @address = @user.addresses.create!(address: '123 Main St', city: 'Denver', state: 'CO', zip: 80218)
      @order_1 = @user.orders.create!(status: 1, address: @address)
      @order_2 = @user.orders.create!(status: 0, address: @address)
      @order_3 = @user.orders.create!(status: 3, address: @address)
      @order_4 = @user.orders.create!(status: 2, address: @address)
    end
